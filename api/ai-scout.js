...
function fetchOgImage(url, timeoutMs = 3500) {
  return new Promise((resolve) => {
    try {
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": "Mozilla/5.0 (compatible; PrintGenieBot/1.0; +https://printgenieai.com)",
      ...
      const r = lib.request(opts, (resp) => {
        ...
        let html = "";
        let size = 0;
        const maxSize = 200 * 1024; // only read first 200 KB — meta tags are at top
        resp.on("data", chunk => {
          size += chunk.length;
          if (size > maxSize) {
            resp.destroy();
          ...
          // Early exit: stop reading once we have <head>...</head>
...
function extractOgImage(html, baseUrl) {
  if (!html) return null;
  // Try og:image first, then twitter:image, then first big <img>
  const patterns = [
...
async function enrichWithOgImages(models, maxParallel = 6, totalBudgetMs = 8000) {
  ...
  for (let i = 0; i < out.length; i += maxParallel) {
    ...
    if (remaining < 500) break; // out of budget
    const batch = out.slice(i, i + maxParallel);
    const perItem = Math.min(3500, Math.max(1500, Math.floor(remaining / Math.ceil((out.length - i) / maxParallel))));
  ...
  try {
    ...
    try {
      ...
      if (mode === "models" && Array.isArray(parsed.models) && parsed.models.length) {
        try {
          parsed.models = await enrichWithOgImages(parsed.models, 6, 8000);
